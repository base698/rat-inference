...
uv run generate-dataset.py --reference ref.jpg --prompt "Add a rat in center" --dataset rat --validate
```
...
# Train model
uv run python train.py --model-size n --epochs 100 --imgsz 640

# Run inference
uv run python inference.py --input test.jpg --model runs/best.pt --imgsz 640
```
...
# Generate data
uv run generate-dataset.py --reference ref.jpg --prompts-file prompts.txt --dataset rat --count 10
...
```bash
uv run --extra dataset python generate-dataset.py --help
```
