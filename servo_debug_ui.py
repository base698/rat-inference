...
class ServoDebugUI:
    def __init__(self, root):
        ...
        # Keyboard control state for arrow keys
        self.keyboard_control_active = False
        self.yaw_motor_id = 1  # Motor ID 1 for yaw
        self.pitch_motor_id = 2  # Motor ID 2 for pitch
        self.yaw_position = 2500  # Center of 1500-3500 range
        self.pitch_position = 300  # Middle of 0-600 range
    ...
    def setup_programming_tab(self):
        ...
        # Camera tracking control frame
        camera_frame = ttk.LabelFrame(self.prog_tab, text="Camera Tracking Control", padding="10")
        camera_frame.pack(fill="x", padx=5, pady=5)

        ttk.Label(camera_frame, text="Arrow Keys Control:").grid(row=0, column=0, columnspan=4, sticky="w")
        ttk.Label(camera_frame, text="← / → - Yaw (1500-3500)").grid(row=1, column=0, columnspan=2, sticky="w", padx=(20, 0))
        ttk.Label(camera_frame, text="↑ / ↓ - Pitch (0-600)").grid(row=2, column=0, columnspan=2, sticky="w", padx=(20, 0))
        ttk.Label(camera_frame, text="C - Center servos").grid(row=3, column=0, columnspan=2, sticky="w", padx=(20, 0))

        # Toggle button for keyboard control
        self.keyboard_btn = ttk.Button(
            camera_frame,
            text="Enable Arrow Key Control",
            command=self.toggle_keyboard_control
        )
        self.keyboard_btn.grid(row=4, column=0, columnspan=2, pady=10)

        # Status display for yaw and pitch
        ttk.Label(camera_frame, text="Yaw (ID 1):").grid(row=1, column=2, sticky="w", padx=20)
        self.yaw_label = ttk.Label(camera_frame, text="2500")
        self.yaw_label.grid(row=1, column=3, sticky="w")

        ttk.Label(camera_frame, text="Pitch (ID 2):").grid(row=2, column=2, sticky="w", padx=20)
        self.pitch_label = ttk.Label(camera_frame, text="300")
        self.pitch_label.grid(row=2, column=3, sticky="w")
    ...
    def setup_keyboard_bindings(self):
        """Set up keyboard bindings for arrow key control"""
        ...
        self.root.bind("<c>", self.on_center_key)
        self.root.bind("<C>", self.on_center_key)

    def toggle_keyboard_control(self):
        """Toggle keyboard arrow control on/off"""
        self.keyboard_control_active = not self.keyboard_control_active

        if self.keyboard_control_active:
            self.keyboard_btn.config(text="Disable Arrow Key Control")
            messagebox.showinfo(
                "Keyboard Control Enabled",
                "Arrow keys are now controlling:\n"
                "← / → - Yaw (Motor ID 1)\n"
                "↑ / ↓ - Pitch (Motor ID 2)\n"
            ...
            self.keyboard_btn.config(text="Enable Arrow Key Control")
    ...
    def on_arrow_key(self, event):
        ...
        step = 50  # Step size for each key press

        try:
            if event.keysym == "Left":
                # Yaw left
                self.yaw_position = max(1500, self.yaw_position - step)
                self.write_motor_position(self.yaw_motor_id, self.yaw_position)
                self.yaw_label.config(text=str(self.yaw_position))
                print(f"Yaw ← {self.yaw_position}")

            elif event.keysym == "Right":
                # Yaw right
                self.yaw_position = min(3500, self.yaw_position + step)
                self.write_motor_position(self.yaw_motor_id, self.yaw_position)
                self.yaw_label.config(text=str(self.yaw_position))
                print(f"Yaw → {self.yaw_position}")

            elif event.keysym == "Up":
                # Pitch up (decrease value)
                self.pitch_position = max(0, self.pitch_position - step)
                self.write_motor_position(self.pitch_motor_id, self.pitch_position)
                self.pitch_label.config(text=str(self.pitch_position))
                print(f"Pitch ↑ {self.pitch_position}")

            elif event.keysym == "Down":
                # Pitch down (increase value)
                self.pitch_position = min(600, self.pitch_position + step)
                self.write_motor_position(self.pitch_motor_id, self.pitch_position)
                self.pitch_label.config(text=str(self.pitch_position))
                print(f"Pitch ↓ {self.pitch_position}")
    ...
    def on_center_key(self, event):
        """Handle 'C' key press to center servos"""
        if not self.keyboard_control_active:
            return
        ...
        try:
            # Center yaw (middle of 1500-3500)
            self.yaw_position = 2500
            self.write_motor_position(self.yaw_motor_id, self.yaw_position)
            self.yaw_label.config(text=str(self.yaw_position))

            # Center pitch (level position)
            self.pitch_position = 0
            self.write_motor_position(self.pitch_motor_id, self.pitch_position)
            self.pitch_label.config(text=str(self.pitch_position))

            print("Servos centered: Yaw=2500, Pitch=0")

        except Exception as e:
            print(f"Error centering servos: {e}")

    def write_motor_position(self, motor_id, position):
        """Write position directly to a motor by ID"""
        try:
            # Check if we have this motor in our discovered motors
            motor_name = None
            for name, motor in self.discovered_motors.items():
                if motor.id == motor_id:
                    motor_name = name
            ...
            if motor_name:
                # Use the motor name to write
                self.motor_bus.write("Goal_Position", motor_name, int(position), normalize=False)
            else:
                # Try direct write if motor not in discovered list
                # This uses the raw packet handler for direct communication
                if hasattr(self.motor_bus, 'packet_handler') and hasattr(self.motor_bus, 'port_handler'):
                    # Address 42 is Goal_Position for STS3215
                    self.motor_bus.packet_handler.write2ByteTxRx(
                        self.motor_bus.port_handler,
                        motor_id,
                        42,  # Goal_Position register
                        int(position)
                    )
                else:
                    print(f"Warning: Motor ID {motor_id} not found in discovered motors")

        except Exception as e:
            print(f"Error writing to motor {motor_id}: {e}")
