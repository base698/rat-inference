...
class OverlayRendererTests(unittest.TestCase):
    ...
    def test_render_hides_tentative_track_boxes(self):
        renderer = self.make_renderer()
        frame = np.zeros((240, 320, 3), dtype=np.uint8)
        ...
        renderer.render(
            ...
            center_point=None,
            tracks=[
                {"id": 4, "bbox": (10, 30, 50, 80), "selected": False, "status": "tentative"},
                {"id": 9, "bbox": (80, 40, 130, 100), "selected": True, "status": "confirmed"},
            ],
        )

        self.assertFalse(np.any(frame[30:81, 10:51]))
        self.assertTrue(np.any(frame[40:101, 80:131]))

    def test_render_hides_stale_world_track_boxes(self):
        renderer = self.make_renderer()
        frame = np.zeros((240, 320, 3), dtype=np.uint8)
        ...
        self.assertFalse(np.any(frame[30:81, 10:51]))
        self.assertFalse(np.any(frame[40:101, 80:131]))
